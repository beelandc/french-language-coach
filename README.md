...
| GET | `/grammar/recommendations/` | Recommend grammar lessons for a conversation feedback focus_area. Query parameters: `focus_area` (required, string from session feedback). Maps the focus_area to matching grammar lesson topics via a bilingual (English + French) keyword dictionary and returns up to 3 recommended lesson summaries (id, title, topic, difficulty). Unknown, empty, or whitespace-only focus areas return a fallback of general beginner lessons instead of an error. Response: `focus_area`, `matched_topics` (list of matched topic strings, empty on fallback), `lessons` (list of up to 3 LessonSummary) |
